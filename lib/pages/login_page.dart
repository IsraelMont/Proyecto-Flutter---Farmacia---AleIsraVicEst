import 'package:farmaciaapp/pages/home_page.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  bool isPasswordVisible =
      false; // Nueva variable para controlar la visibilidad de la contraseña
  bool obscureText = true; // Variable para ocultar la contraseña
      ...
      decoration: BoxDecoration(
        ...
        image: DecorationImage(
          ...
          colorFilter: ColorFilter.mode(myColor.withOpacity(0.2),
              BlendMode.dstATop), // Aplica un filtro de color a la imagen
        ),
      ),
      child: Scaffold(
        backgroundColor:
            Colors.transparent, // Hace transparente el fondo del Scaffold
        body: Stack(children: [
          Positioned(
              top: 80, child: _buildTop()), // Posiciona la parte superior
          Positioned(
              bottom: 0, child: _buildBottom()), // Posiciona la parte inferior
        ]),
      ),
    );
  }
      ...
      child: Card(
        shape: const RoundedRectangleBorder(
          ...
          topLeft: Radius.circular(
              30), // Bordes redondeados en la parte superior izquierda
          topRight: Radius.circular(
              30), // Bordes redondeados en la parte superior derecha
        )),
        child: Padding(
  ...
  Widget _buildInputField(TextEditingController controller,
    ...
    return TextField(
      controller: controller, // Controlador del campo de entrada
        ...
        suffixIcon: isPassword
            ? IconButton(
                icon: Icon(
                  isPasswordVisible
                      ? Icons.visibility
                      : Icons.visibility_off, // Ícono de visibilidad
                ),
                onPressed: () {
                  setState(() {
                    isPasswordVisible =
                        !isPasswordVisible; // Cambia el estado de visibilidad
                  });
                },
              )
            : const Icon(Icons.done), // Ícono normal para otros campos
      ),
      obscureText: isPassword &&
          !isPasswordVisible, // Oculta el texto si es contraseña y no está visible
    );
  }
  ...
  void _login() {
    ...
    if (emailController.text == predefinedCredentials.email &&
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
            builder: (context) =>
                const HomePage()), // Navega a la página principal
      );
    } else {
    ...
    return Center(
      child: Column(
        children: [
          ...
          Row(
            mainAxisAlignment:
                MainAxisAlignment.spaceEvenly, // Espaciado entre elementos
            children: [
              Tab(
                  icon: Image.asset(
                      "assets/images/facebook.png")), // Opción de Facebook
              Tab(icon: Image.asset("assets/images/x.png")), // Opción de x
              Tab(
                  icon: Image.asset(
                      "assets/images/google.png")), // Opción de Google
            ],
          )
        ],
      ),
